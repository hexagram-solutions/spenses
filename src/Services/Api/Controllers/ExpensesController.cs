...
public class ExpensesController(ISender sender) : ControllerBase
{
    ...
    {
        var expense = await sender.Send(new CreateExpenseCommand(homeId, props));
    ...
    {
        var expenses = await sender.Send(new ExpensesQuery(homeId)
        {
            Skip = query.Skip,
            Take = query.Take,
    ...
    {
        var expense = await sender.Send(new ExpenseQuery(homeId, expenseId));
    ...
    {
        var expenses = await sender.Send(new UpdateExpenseCommand(homeId, expenseId, props));
    ...
    {
        await sender.Send(new DeleteExpenseCommand(homeId, expenseId));
    ...
    {
        var filters = await sender.Send(new ExpenseFiltersQuery(homeId));
